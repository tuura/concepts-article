...
%TODO: 1-3 recent references [X] by lead research groups to support
%the opening statement. Talk to Alex & Danil.
...
%TODO: ref [V] for Versify tool by Oriol Roig
%TODO: ref [W] to Workcraft website
%TODO: ref [Y] to ASYNC paper on multi-phase buck
...
\noun{Workcraft}~\cite{2007_poliakov_workcraft}[W], and others.
...
consistent specification is a major design challenge~\cite{2014_sokolov_ftfc}[Y].
...
\begin{itemize}
  \item We formally introduce asynchronous concepts as a specification
  language (\S\ref{sec:Concepts}).
  \item We show how to use concepts for circuit specification
  (\S\ref{sec:Circuit-specification-with}) and automatically translate
  concepts to STGs (\S\ref{sec: interop-with-stg}) for verification and 
  synthesis using our open-source software tool \noun{Workcraft}.
  \item We demonstrate the proposed design method on a case study of an
  asynchronous power regulator (\S\ref{sec:case-study}).
\end{itemize}

\noindent
We start with a motivational example in \S\ref{} and review related work
in \S\ref{sec:related-work}.
...
allows the inputs to transition in the opposite \\
ion to the output
...
\begin{minipage}[t]{1\columnwidth}%
$\mathsf{circuit}\,a \,b \,c=\mathsf{handshake00}\,a \,c \,\diamond\,\mathsf{handshake00}\,b \,c$%
\end{minipage}\vspace{2mm}
