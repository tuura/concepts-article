...
The concepts tool, which translates concepts to STGs has been integrated into \noun{Workcraft}. This allows a designer to
visualise their concept designs as STGs, to simulate, verify and synthesise them using tools also integrated in \noun{Workcraft},
and if there are any corrections or additions to be made these can be done either
directly in the STG or in the original concept specification, which can then
be re-translated into an updated STG. These automated processes can allow for a streamlined desing process.

Scenario combination is not yet implemented. This information gives guidelines on how one can perform it manually, however this process can be automated to further
streamline the design process using concepts.
...
~~~~\mathsf{uvReact'}&\hspace{-2mm}=&\hspace{-2mm}zc^{+}\rightsquigarrow uv^{+}~\diamond~zc^{-}\rightsquigarrow uv^{-}\diamond~gp\_ack^{+}\rightsquigarrow uv^{-}
...
The scenario STGs can now be combined as described in \S\ref{sub:scenario-composition}
...
It is also possible to use custom properties as verification tools, by entering
the code to check for these.
Figure~\ref{fig:custom_assertion} contains an example custom assertion we
...
digital, data featuring multiple bits, and little-digital, control systems. A Balsa specification is initially converted into a format describing a network of handshaked components, which can 
be used for simulation and circuit diagrams. This can then be used in synthesis, by mapping handshaked components on to library components~\cite{van1993handshake}. RTL languages
...
it into operational modes, scenarios, and describing signal interactions and requirements
of the scenario in a textual format. These can then be translated into STGs
that represent these operational modes, which can be used with existing verification and synthesis tools. 
These can be combined to produce a model for the full system specification.
...
\emph{Process mining} can also be used for various purposes in conjunction with designing
...
complex models. All of this can be performed automatically, by tools such as \noun{PGminer}~\cite{pgminer}, and aid a
designer in reducing design time and errors. We aim to include process mining in the design flow for the proposed approach.

We aim to test concepts using examples from several different areas where asynchronous circuits are beneficial. 
...
to increase the operation time of these devices. We beleive concepts 
