...
of a C-element with an environment, any representation mentioned in Section~\ref{sub:Comp-of-concepts} when passed into this algorithm would initially produce the list of concepts found in 
...
token, being present if $a^{-}$ has occured, absent if it has yet to occur. This is connected to the the transition of $c^{+}$. We use a read-arc to connect these, as if $a_0$ contains a 
token, a read-arc will allow $c^{+}$ to transition without consuming the token, because this would block $a-$ from occurring.
...
Using the example in Section~\ref{sub:Comp-of-concepts}, a C-element with environment, we can convert the $\mathsf{cElement}(e)$ and the two $\mathsf{inverter}(e)$ concepts individually. Including the initial 
states, this would give us the STGs found in Figure~\ref{fig:IndividConceptStgs}.
...
is implied at conversion time by consistency loops.
