...
IoT computation nodes. Power management units interface directly with the
analogue world and can benefit from asynchronous implementation that allows
them to react to changes in the system at the rate they occur, instead of
sampling various analogue parameters by clock, which is both slow and
energy-wasteful~\cite{2014_sokolov_ftfc}.

In this section we provide background on asynchronous buck converters, and
discuss challenges arising when designing asynchronous controllers using
Signal Transition Graphs~(STGs)~\cite{Chu_1987_phd}\cite{Rosenblum_1985_tpn},
a common model for the specification of asynchronous circuits.

% A monolithic
% design approach is used when designing with STGs, which can become problematic
% as designs become larger.

% [...] system is an illustrative example: it relies
% on analogue circuitry for power regulation and conversion, and its behaviour
% is characterised by many operating modes with complex interplay and
% high-level decision logic that is digitally controlled.
% Asynchronous circuits are event-driven, i.e. they react to changes

% In this section we introduce the subject of the case study we will use to demonstrate the design flow for concepts in Section~\ref{sec:case-study}. We will then discuss the monolithic
% design approach that has been used to design this, and it's disadvantages. Following this, we will explore a way of viewing STGs which has helped us develop concepts.

\subsection{Background on buck converters}

The example we will use in the case study of this article is from the power management domain, a \emph{Multi-scenario power regulator}~\cite{2014_sokolov_ftfc}.
A basic power regulator comprises an analog buck and a digital controller,
...
is explicitly notified~(by the \textsf{$gp\_ack$} and \textsf{$gn\_ack$} signals)
...
With the information provided here, a designer can then work to produce a formal specification using the monolithic approach. Figure~\ref{fig:Monolithic-buck} contains a completed STG specification for a simple buck controller.
...
This STG is quite large, and has several overlayed scenarios. The monolithic approach starts with a blank page, and a designer would have to manually insert signals and the
connections between these according to the description of operation of the system. With there being several scenarios, the designer could choose to design each separately, and
manually connect these depending on the similarities between these scenarios.
...
debug and edit. This can slow the design process of a circuit, which is undesirable in an industry where the time for a device to move from conception to market is becoming critical, and ever shorter~\cite{2014_sokolov_ftfc}.

\subsection{A new way of viewing STGs \label{sub:new-way}}
In an attempt to streamline the design process, we aimed to find a way to create STGs which allows editing at various stages of the design. This way, if something needs to be corrected,
this can be done to a small part, but we can reuse everything that works correctly. These can then be composed to produce a full STG which contains the corrections, with a minimum
...
\begin{enumerate}
\item High levels of \textsf{$uv$} and \textsf{$oc$} are mutually exclusive
\item High levels of \textsf{$gp$} and \textsf{$gn$} are mutually exclusive
...
these signal will be subject to these protocols, which will prevent circuit breaking bugs in the testing phase. For example, an
interaction involving \textsf{$gp$} will ensure that it is never set high at the same time as \textsf{$gn$}, as otherwise the mutual exclusion will not hold.
