...
Our contributions are as follows: (i) we formally introduce asynchronous
concepts as a specification language (\S\ref{sec:Concepts}), (ii) we
show how to use concepts for circuit specification
(\S\ref{sec:Circuit-specification-with}) and automatically translate
them to STGs (\S\ref{sec: interop-with-stg}), and (iii) we illustrate
the proposed design method on a case study of an asynchronous power
regulator (\S\ref{sec:case-study}).
